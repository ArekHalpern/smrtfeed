...
export function AddPaperModal({
  ...
  const handlePDFExtract = (data: ExtractedPDFData) => {
    // Combine all sentences from all pages into a single string
    const allText = data.pages
      .flatMap((page) => page.sentences.map((sentence) => sentence.content))
      .join(" ");
